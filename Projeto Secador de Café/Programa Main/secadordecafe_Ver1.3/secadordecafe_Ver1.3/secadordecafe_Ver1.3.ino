...
float sensor_read = 0; // variable used to read the sensor value and display it
  /***********/
...
void loop() {
  ...
  lcd.print("Time:        Sec");
  lcd.setCursor(6,1); // colum and line
  lcd.print(sec);
  lcd.setCursor(0,0); // colum and line
  lcd.print("Temp.:        oC");
  
  lcd.setCursor(7,0); // colum and line
...
ISR(TIMER1_OVF_vect)                              //interrupção do TIMER1 
{
  ...
  sensor_read =  (float(analogRead(sensor))*5/(1023))/0.01; // this line makes the analogRead and the conversion to the actual temperature
                // Here is explained why is that math needed:
                // V0 = (read value at A0)*(5Volts/1023)
                // Actual Temperature = V0/10mVolts
