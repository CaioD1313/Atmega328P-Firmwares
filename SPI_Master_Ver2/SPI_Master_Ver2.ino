...
if(digitalRead(reset) == HIGH) // This Button Will Call SPI_Com();
{
  ...
  delay(200);
  lcd.blink();
  Rx = SPI_Com(aux); // sends aux as a byte to Slave to make the transform; (receives it as an int variable so it can be easily writen on the LCD)
...
byte SPI_Com(byte send)
  {
    ...
    for(k = 0x80; k != 0; k /= 2) // \frac{k}{2} shifts the kth bit to the right >>
    {
      ...
      if(digitalRead(MISO) == HIGH) // if the Master Input is in HIGH state (a bit has been sent), then stores this bit in the kth position of the received var
        {received |= k ;}  
